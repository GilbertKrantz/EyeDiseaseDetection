...
class DataHandler:
    def __init__(self, data_path: str, batch_size: int = 32, num_workers: int = 4, data_map: dict = None):
        self.data_path = data_path
        self.batch_size = batch_size
        self.num_workers = num_workers
        self.data_map = data_map if data_map else {  
    ...
    def load_data(self) -> dict:
        ...
        for split, path in self.data_map.items():
            ...
            DataLoaders[split] = DataLoader(dataset, batch_size=self.batch_size, shuffle=True if split == 'train' else False, num_workers=self.num_workers)
        return DataLoaders
...
class ModelHandler:
    def __init__(self, model_name: str, dataloaders: dict, config_path: str = None):
        ...
        self.history = None
        self.hyper_params = self.config.get('hyper_params', {
            'learning_rate': 0.001,
            'weight_decay': 1e-5
        })
    ...
    def initialize_model(self, model_name: str = None, config: dict = None) -> EyeDiseaseClassifier:
        """
    ...
    def load_model(self, model_path: str, config_path: str = None):
        ...
        if self.config:
            self.model = self.initialize_model(model_name=self.model.model_name, config=self.config)
    ...
    def inference(self, image_path: str = None, image: Image.Image = None, preprocess: callable = None):
        """
    ...
    def batch_inference(self, image_paths: list = None, images: list = None, preprocess: callable = None):
        """
